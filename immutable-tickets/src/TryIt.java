...
public class TryIt {
    ...
    public static void main(String[] args) {
        ...
        // Since object is immutable, we must capture returned object

        t = service.assign(t, "agent@example.com");
        t = service.escalateToCritical(t);

        System.out.println("\nAfter service updates (new object created): " + t);

        // Try external mutation (will fail because list is unmodifiable)
        try {
            List<String> tags = t.getTags();
            tags.add("HACKED_FROM_OUTSIDE");
        } catch (Exception e) {
            System.out.println("\nExternal tag modification blocked (immutable).");
        }
